...
type AuthService interface {
	...
	NewContact(userID, contactID string) error
	GetContacts(userID, contactID string) (*[]Contact, error)
...
func (s *AuthServiceImpl) GetContacts(userID, contactID string) (*[]Contact, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	resp, err := s.repo.GetContacts(ctx, userID, contactID)
	if err != nil {
		return nil, err
	}
...
func (s *AuthServiceImpl) NewContact(userID, contactID string) error {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	err := s.repo.NewContact(ctx, userID, contactID)
	if err != nil {
		return err
	}
