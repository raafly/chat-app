...
type AuthRepo interface {
	...
	NewContact(ctx context.Context, userID, contactID string) error
	GetContacts(ctx context.Context, userID, contactID string) (*[]Contact, error)
	GetHistory(ctx context.Context, userIO, contactID string) (*[]Message, error)
}
...
func (a *AuthRepoImpl) GetContacts(ctx context.Context, userID, contactID string) (*[]Contact, error) {
	rows, err := a.db.QueryContext(ctx, "SELECT id, user_id, contact_id FROM contacts WHERE user_id = ? AND contact_id =?", userID, contactID)
	if err != nil {
		log.Println(err)
...
func (a *AuthRepoImpl)NewContact(ctx context.Context, userID, contactsID string) error {
	_, err := a.db.ExecContext(ctx, "insert into contacts(user_id, contact_id) VALUES(?, ?)", userID, contactsID)

	if err != nil {
		return helper.ErrBadRequest("duplicate contact", nil)
	}
